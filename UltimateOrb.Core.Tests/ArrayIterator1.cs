...
namespace UltimateOrb.Core.Tests.B {
    using mp_ptr = ManagedPtr<mp_limb_t>;
    using mp_srcptr = ReadOnlyManagedPtr<mp_limb_t>;
...
mpn_toom54_mul_itch(mp_size_t an, mp_size_t bn) {
        ...
        static void MPN_COPY_INCR(mp_ptr dst, mp_srcptr src, mp_size_t n) {
            ...
            if ((n) != 0) {
                ...
                mp_srcptr __src = (src);
                mp_limb_t __x;
                __x = __src++.Dereferenced;
                if (__n != 0) {
                    do {

                        __dst++.Dereferenced = __x;
                        __x = __src++.Dereferenced;
                    }
                    while (0 != --__n);
                }
        ...
        static void MPN_REVERSE(mp_ptr dst, mp_srcptr src, mp_size_t size) {
            ...
            for (__i = 0; __i < __size; __i++) {
                ...
                __dst.Dereferenced = __src.Dereferenced;
                __dst++;
                __src--;
            }
        }
...
mpn_com(mp_ptr rp, mp_srcptr up, mp_size_t n) {
            ...
            do {
                ul = up++.Dereferenced;
                rp++.Dereferenced = ~ul & GMP_NUMB_MASK;
            } while (--n != 0);
        }
...
mpn_sub_nc(mp_ptr rp, mp_srcptr up, mp_srcptr vp, mp_size_t n, mp_limb_t ci) {
 ...
 mpn_neg(mp_ptr __gmp_rp, mp_srcptr __gmp_up, mp_size_t __gmp_n) {
            while (__gmp_up.Dereferenced == 0) /* Low zero limbs are unchanged by negation. */
              {
                __gmp_rp.Dereferenced = 0;
                if (0 == --__gmp_n) /* All zero */
                    return 0;
                ++__gmp_up; ++__gmp_rp;
            }

            __gmp_rp.Dereferenced = (NEG(__gmp_up.Dereferenced)) & GMP_NUMB_MASK;
...
mpn_sqr_basecase(mp_ptr rp, mp_srcptr up, mp_size_t n) {
        ...
        public static void mpn_incr_u(mp_ptr p, mp_limb_t incr) {
            ...
            mp_ptr __p = (p);
            {
                __x = __p.Dereferenced + (incr);

                __p.Dereferenced = __x;
                if (__x < (incr))
                    while (++((++__p).Dereferenced) == 0)
                        ;
            }
        ...
        public static void mpn_decr_u(mp_ptr p, mp_limb_t incr) {
            ...
            mp_ptr __p = (p);
            {
                __x = __p.Dereferenced;

                __p.Dereferenced = __x - (incr);
                if (__x < (incr))
                    while (((++__p).Dereferenced)-- == 0)
                        ;
            }
        ...
        */
        ...
        mpn_lshift(mp_ptr rp, mp_srcptr up, mp_size_t n, int cnt) {
            ...
            low_limb = (--up).Dereferenced;
            retval = low_limb >> tnc;
            high_limb = (low_limb << cnt) & GMP_NUMB_MASK;

            for (i = n - 1; i != 0; i--) {
                low_limb = (--up).Dereferenced;
                (--rp).Dereferenced = high_limb | (low_limb >> tnc);
                high_limb = (low_limb << cnt) & GMP_NUMB_MASK;
            }
            (--rp).Dereferenced = high_limb;
       ...
       mpn_rshift(mp_ptr rp, mp_srcptr up, mp_size_t n, int cnt) {
            ...
            tnc = GMP_NUMB_BITS - cnt;
            high_limb = up++.Dereferenced;
            retval = (high_limb << tnc) & GMP_NUMB_MASK;
            low_limb = high_limb >> cnt;

            for (i = n - 1; i != 0; i--) {
                high_limb = up++.Dereferenced;
                rp++.Dereferenced = low_limb | ((high_limb << tnc) & GMP_NUMB_MASK);
                low_limb = high_limb >> cnt;
            }
            rp.Dereferenced = low_limb;
...
void mpn_divexact_by15(mp_ptr dst, mp_srcptr src, mp_size_t size) {
    ...
    mpn_fft_best_k_A(mp_size_t n, int sqr) {
            ...
            last_k = fft_tab.Dereferenced.k;
            for (tab = fft_tab + 1; ; tab++) {
                tab_n = tab.Dereferenced.n;
                thres = tab_n << last_k;
                if (n <= thres)
                    break;
                last_k = tab.Dereferenced.k;
            }
            return last_k;
        }
