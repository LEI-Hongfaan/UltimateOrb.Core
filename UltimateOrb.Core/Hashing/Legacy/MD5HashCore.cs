...
namespace UltimateOrb.Hashing.Legacy {
    ...
    internal unsafe struct MD5HashCore {
        ...
        private static bool GetIsLittleEndianness() {
            var bufferBytes = (stackalloc byte[sizeof(UInt32)]);
            for (int i = 0; bufferBytes.Length > i; ++i) {
                bufferBytes[i] = (byte)i;
            }
            var bufferUInt32 = MemoryMarshal.Cast<byte, UInt32>(bufferBytes);
            var result = (0X03020100U == bufferUInt32[0]);
            if (!result) {
                if (0X00010203U != bufferUInt32[0]) {
...
#pragma warning restore HAA0502 // Explicit new reference type allocation
        ...
        private static Vector128<UInt32> ComputeHashCode<TReverseEndianness>(ReadOnlySpan<UInt32> source, Vector128<UInt32> hashCode)
            ...
            unchecked {
                ...
                for (; 16 <= source.Length; source = source[16..]) {
        ...
        private void ComputeFinalHashCode_BitLength(Span<byte> buffer) {
            BinaryPrimitives.WriteUInt32LittleEndian(buffer[56..], unchecked((UInt32)(_BytePosition * 8))); // 8 bits
            BinaryPrimitives.WriteUInt32LittleEndian(buffer[60..], checked((UInt32)(_BytePosition >> 29)));
        }
        ...
        public void ComputeHashCode(ReadOnlySpan<byte> source) {
            ...
            if (0 != r) {
                ...
                if (source.Length <= p) {
                    ...
                    source[..p].CopyTo(buffer[r..]);
                    sourceRest = source[p..];
                    hashCode = ComputeHashCode(buffer, hashCode);
                }
            }
            var b = ~63 & sourceRest.Length;
            _HashCode = ComputeHashCode(sourceRest[..b], hashCode);
            sourceRest[b..].CopyTo(buffer);
            _BytePosition = bytePosition + unchecked((uint)source.Length);
        }
